...
const THEME_CONFIGS = {
  rose: {
    ...
    name: 'ורוד בייבי',
    colorHex: '#fb7185',
    primaryBtn: 'bg-rose-400 text-white shadow-rose-200/50 hover:bg-rose-500',
    ...
    activeBtn: 'bg-rose-400 text-white shadow-md shadow-rose-200/50'
  },
  teal: {
    id: 'teal',
    name: 'טורקיז מנטה',
    colorHex: '#14b8a6',
    primaryBtn: 'bg-teal-500 text-white shadow-teal-200/50 hover:bg-teal-600',
    ...
    activeBtn: 'bg-teal-500 text-white shadow-md shadow-teal-200/50'
  },
  purple: {
    id: 'purple',
    name: 'סגול לבנדר',
    colorHex: '#a855f7',
    primaryBtn: 'bg-purple-500 text-white shadow-purple-200/50 hover:bg-purple-600',
    ...
    activeBtn: 'bg-purple-500 text-white shadow-md shadow-purple-200/50'
  },
  amber: {
    id: 'amber',
    name: 'כתום שקיעה',
    colorHex: '#f59e0b',
    primaryBtn: 'bg-amber-500 text-white shadow-amber-200/50 hover:bg-amber-600',
    activeTab: 'text-amber-500 font-extrabold',
    badge: 'bg-amber-100 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300',
    headerGrad: 'from-amber-400 via-orange-300 to-rose-300',
    activeBtn: 'bg-amber-500 text-white shadow-md shadow-amber-200/50'
  }
};

const BOTTLE_PRESETS = [60, 90, 120, 150, 180, 210, 240];
const QUICK_SOLIDS = ['בטטה', 'גזר', 'קישוא', 'דלעת', 'בננה', 'אבוקדו', 'טחינה', 'תפוח', 'אגס', 'עדשים', 'עוף'];

export default function App() {
  const [session, setSession] = useState(() => {
    const saved = localStorage.getItem('bobo_session_user');
    return saved ? JSON.parse(saved) : { email: 'oren@bobo.app', id: 'local-user' };
  });
  ...
  const [feedings, setFeedings] = useState(() => JSON.parse(localStorage.getItem('bobo_feedings') || localStorage.getItem('feedings') || '[]'));
  const [diapers, setDiapers] = useState(() => JSON.parse(localStorage.getItem('bobo_diapers') || localStorage.getItem('diapers') || '[]'));
  const [solids, setSolids] = useState(() => JSON.parse(localStorage.getItem('bobo_solids') || localStorage.getItem('solids') || '[]'));
  const [growthMetrics, setGrowthMetrics] = useState(() => JSON.parse(localStorage.getItem('bobo_growth') || localStorage.getItem('growth_metrics') || '[]'));
  ...
  const [notificationsEnabled, setNotificationsEnabled] = useState(() => typeof window !== 'undefined' && 'Notification' in window && window.Notification.permission === 'granted');
  ...
  useEffect(() => {
    ...
    if (significantFeedings.length > 0) {
      setLastFeedTime(new Date(significantFeedings[0].created_at || significantFeedings[0].timestamp).getTime());
    } else {
  ...
  useEffect(() => {
    if (!lastFeedTime) { setTimeToNextFeed(null); return; }
    const targetTime = lastFeedTime + (feedIntervalHours * 60 * 60 * 1000);
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    const userObj = { email: authEmail || 'oren@bobo.app', id: 'bobo-admin' };
    setSession(userObj);
    localStorage.setItem('bobo_session_user', JSON.stringify(userObj));
  };

  const handleLogout = () => {
    localStorage.removeItem('bobo_session_user');
    setSession(null);
  };
  ...
  const handleImportBackup = (e) => {
    ...
    fileReader.onload = async (event) => {
      try {
        ...
        setFeedings(f); saveLocal('feedings', f);
        setDiapers(d); saveLocal('diapers', d);
        setSolids(s); saveLocal('solids', s);
        setGrowthMetrics(g); saveLocal('growth_metrics', g);
        if (data.babyName) { setBabyName(data.babyName); localStorage.setItem('bobo_name', data.babyName); }

        alert(`✅ הגיבוי נטען בהצלחה! כל הנתונים שוחזרו במלואם.`);
      } catch (err) {
  ...
  const saveGrowth = () => {
    ...
    saveLocal('growth_metrics', updated);
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl w-full max-w-sm space-y-6 text-center border border-gray-100 dark:border-gray-700">
          <div className="w-16 h-16 bg-gradient-to-tr from-rose-400 to-amber-300 rounded-3xl mx-auto flex items-center justify-center text-3xl shadow-lg">👶</div>
          <div>
            <h1 className="text-2xl font-black text-gray-900 dark:text-white">בובו - יומן תינוק</h1>
            <p className="text-xs text-gray-500 mt-1">התחברות למערכת המעקב</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-3">
            <input type="email" value={authEmail} onChange={e => setAuthEmail(e.target.value)} placeholder="אימייל" className="w-full p-3.5 border rounded-2xl text-xs font-bold bg-gray-50 dark:bg-slate-700" />
            <input type="password" value={authPassword} onChange={e => setAuthPassword(e.target.value)} placeholder="סיסמה" className="w-full p-3.5 border rounded-2xl text-xs font-bold bg-gray-50 dark:bg-slate-700" />
            <button type="submit" className="w-full py-4 bg-rose-400 hover:bg-rose-500 text-white font-black rounded-2xl text-sm shadow-lg shadow-rose-200/50">כניסה למערכת</button>
          </form>
        </div>
      </div>
    );
          ...
          <>
            ...
            <div className="bg-white dark:bg-gray-800 p-4 rounded-3xl flex justify-between items-center shadow-md border border-gray-100 dark:border-gray-700">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[11px] font-bold text-gray-400">האכלה הבאה בעוד (כל {feedIntervalHours} שעות):</p>
              ...
              <div className="bg-white dark:bg-gray-800 p-5 rounded-3xl shadow-lg border space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['left', 'both', 'right'].map(s => (
                    <button key={s} onClick={() => setFeedSide(s)} className={`py-3 rounded-2xl text-sm font-bold ${feedSide === s ? 'bg-rose-400 text-white shadow-md' : 'bg-gray-50 dark:bg-gray-700'}`}>
                      {s === 'left' ? 'שמאל' : s === 'right' ? 'ימין' : 'שניהם'}
                    </button>
                  ))}
                </div>

                <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-700 p-3 rounded-2xl">
                  <div className="flex items-center gap-2">
                    <button onClick={() => setDuration(Math.max(1, duration - 1))} className="w-9 h-9 rounded-xl bg-white dark:bg-gray-800 font-bold shadow-sm">-</button>
                    <span className="font-bold text-base px-2">{duration} דקות</span>
                    <button onClick={() => setDuration(duration + 1)} className="w-9 h-9 rounded-xl bg-white dark:bg-gray-800 font-bold shadow-sm">+</button>
                  </div>
                  <button onClick={() => setIsTimerActive(!isTimerActive)} className={`px-4 py-2 rounded-xl text-xs font-bold ${isTimerActive ? 'bg-red-500 text-white' : 'bg-rose-100 text-rose-600'}`}>
                    {isTimerActive ? 'עצור טיימר ⏸' : 'הפעל טיימר ⏱'}
                  </button>
                </div>

                <button onClick={() => saveFeeding('breastfeeding')} className="w-full font-bold py-3.5 rounded-xl bg-rose-400 text-white shadow-lg shadow-rose-200/50">שמור הנקה</button>
              </div>
            )}

            {/* טופס בקבוק מורחב עם כמויות מהירות */}
              ...
              <div className="bg-white dark:bg-gray-800 p-5 rounded-3xl shadow-lg border space-y-4">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {BOTTLE_PRESETS.map(preset => (
                    <button key={preset} onClick={() => setAmount(preset)} className={`px-3.5 py-2 rounded-xl text-xs font-bold border ${amount === preset ? 'bg-cyan-400 text-white shadow-md' : 'bg-gray-50 dark:bg-gray-700'}`}>
                      {preset} מ"ל
                    </button>
                  ))}
                </div>

                <div className="flex items-center justify-center gap-3 bg-gray-50 dark:bg-gray-700 p-3 rounded-2xl">
                  <button onClick={() => setAmount(Math.max(10, amount - 10))} className="w-9 h-9 rounded-xl bg-white dark:bg-gray-800 font-bold shadow-sm">-10</button>
                  <input type="number" value={amount} onChange={e => setAmount(parseInt(e.target.value || 0))} className="w-24 text-center font-black text-lg bg-transparent" />
                  <span className="font-bold text-xs">מ"ל</span>
                  <button onClick={() => setAmount(amount + 10)} className="w-9 h-9 rounded-xl bg-white dark:bg-gray-800 font-bold shadow-sm">+10</button>
                </div>

                <button onClick={() => saveFeeding('bottle')} className="w-full font-bold py-3.5 rounded-xl bg-cyan-400 text-white shadow-lg shadow-cyan-200/50">שמור האכלה</button>
              </div>
            )}
            ...
            {activeSection === 'pumping' && (
              <div className="bg-white dark:bg-gray-800 p-5 rounded-3xl shadow-lg border space-y-4">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {BOTTLE_PRESETS.map(preset => (
                    <button key={preset} onClick={() => setAmount(preset)} className={`px-3.5 py-2 rounded-xl text-xs font-bold border ${amount === preset ? 'bg-purple-400 text-white shadow-md' : 'bg-gray-50 dark:bg-gray-700'}`}>
                      {preset} מ"ל
                    </button>
                  ))}
                </div>
                ...
                <button onClick={() => saveFeeding('pumping')} className="w-full font-bold py-3.5 rounded-xl bg-purple-400 text-white shadow-lg shadow-purple-200/50">שמור שאיבה</button>
              </div>
            )}
            ...
            {activeSection === 'diaper' && (
              <div className="bg-white dark:bg-gray-800 p-5 rounded-3xl shadow-lg border space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['wet', 'dirty', 'both'].map(t => <button key={t} onClick={() => setDiaperType(t)} className={`py-3.5 rounded-2xl text-sm font-bold ${diaperType === t ? 'bg-amber-400 text-white shadow-md' : 'bg-gray-50 dark:bg-gray-700'}`}>{t === 'wet' ? '💧 פיפי' : t === 'dirty' ? '💩 קקי' : '✨ שניהם'}</button>)}
                </div>
                <button onClick={saveDiaper} className="w-full font-bold py-3.5 rounded-xl bg-amber-400 text-white shadow-lg shadow-amber-200/50">שמור חיתול</button>
              </div>
            )}
            ...
            {activeSection === 'solid' && (
              <div className="bg-white dark:bg-gray-800 p-5 rounded-3xl shadow-lg border space-y-4">
                ...
                <button onClick={saveSolid} className="w-full font-bold py-3.5 rounded-xl bg-emerald-500 text-white shadow-lg">שמור טעימה</button>
              </div>
            )}
        ...
        {activeTab === 'settings' && (
          <div className="bg-white dark:bg-gray-800 p-5 rounded-3xl border space-y-5">
            <div className="flex justify-between items-center border-b pb-2">
              <h2 className="text-base font-bold">⚙️ הגדרות מערכת</h2>
              <button onClick={handleLogout} className="text-xs font-bold text-red-500 bg-red-50 dark:bg-red-950/30 px-2.5 py-1 rounded-xl">התנתק</button>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.values(THEME_CONFIGS).map(th => (
                  <button key={th.id} onClick={() => { setThemePreset(th.id); localStorage.setItem('bobo_theme', th.id); }} className={`p-3 rounded-2xl border text-xs font-bold flex items-center justify-between ${themePreset === th.id ? 'border-gray-800 bg-gray-50 dark:bg-gray-700' : 'border-gray-100'}`}>
                    <div className="flex items-center gap-2">
            ...
            <div className="bg-rose-50/50 dark:bg-rose-950/20 p-4 rounded-2xl border border-rose-100 dark:border-rose-900 space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-xs font-bold text-rose-700 dark:text-rose-300">🔔 התראות דחיפה לנייד</span>
                <button onClick={() => {
                  if (typeof window !== 'undefined' && 'Notification' in window) {
                    Notification.requestPermission().then(perm => setNotificationsEnabled(perm === 'granted'));
                  }
                }} className="text-xs font-bold px-3 py-1.5 rounded-xl bg-teal-500 text-white">
              ...
              <div>
                <label className="block text-xs font-bold mb-1.5 text-gray-700 dark:text-gray-300">
                  מרווח זמן בין האכלות: <span className="text-rose-500 font-extrabold">{feedIntervalHours} שעות</span>
                </label>
                <div className="grid grid-cols-5 gap-1.5">
                  {[2, 2.5, 3, 3.5, 4].map(val => (
                    <button key={val} onClick={() => { setFeedIntervalHours(val); localStorage.setItem('bobo_feed_interval', val); }} className={`py-2 rounded-xl text-xs font-bold ${feedIntervalHours === val ? 'bg-rose-400 text-white shadow-md' : 'bg-white dark:bg-gray-800'}`}>
                      {val}ש׳
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="block text-xs font-bold mb-1.5 text-gray-700 dark:text-gray-300">
                  התראה מראש לפני האכלה: <span className="text-teal-600 font-extrabold">{notifyLeadMinutes} דקות</span>
                </label>
                <div className="grid grid-cols-4 gap-1.5">
                  {[5, 10, 15, 20].map(val => (
                    <button key={val} onClick={() => { setNotifyLeadMinutes(val); localStorage.setItem('bobo_notify_lead', val); }} className={`py-2 rounded-xl text-xs font-bold ${notifyLeadMinutes === val ? 'bg-teal-500 text-white shadow-md' : 'bg-white dark:bg-gray-800'}`}>
                      {val} דק׳
                    </button>
                  ))}
                </div>
            ...
            <div className="space-y-3 border-t pt-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={defaultMl} onChange={e => { const v = parseInt(e.target.value || 0); setDefaultMl(v); localStorage.setItem('bobo_def_ml', v); }} className="w-full p-2 border rounded-xl text-center font-bold" />
                </div>
                <div>
                  <label className="block font-bold mb-1">סוג בקבוק מועדף:</label>
                  <select value={defaultBottleType} onChange={e => { setDefaultBottleType(e.target.value); localStorage.setItem('bobo_def_bottle_type', e.target.value); }} className="w-full p-2 border rounded-xl font-bold bg-white dark:bg-gray-800">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={targetFeeds} onChange={e => { const v = parseInt(e.target.value || 1); setTargetFeeds(v); localStorage.setItem('bobo_target_feeds', v); }} className="w-full p-2 border rounded-xl text-center font-bold" />
                </div>
                <div>
                  <label className="block font-bold mb-1">יעד חיתולים יומי:</label>
                  <input type="number" value={targetDiapers} onChange={e => { const v = parseInt(e.target.value || 1); setTargetDiapers(v); localStorage.setItem('bobo_target_diapers', v); }} className="w-full p-2 border rounded-xl text-center font-bold" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 border-t pt-3 text-xs">
              ...
              <input type="text" value={babyName} onChange={e => { setBabyName(e.target.value); localStorage.setItem('bobo_name', e.target.value); }} placeholder="שם התינוק/ת" className="w-full p-2 border rounded-xl font-bold bg-white dark:bg-gray-800" />
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => { setBabyGender('בן'); localStorage.setItem('bobo_gender', 'בן'); }} className={`p-2 border rounded-xl font-bold ${babyGender === 'בן' ? 'bg-cyan-400 text-white' : 'bg-white dark:bg-gray-800'}`}>בן</button>
                <button onClick={() => { setBabyGender('בת'); localStorage.setItem('bobo_gender', 'בת'); }} className={`p-2 border rounded-xl font-bold ${babyGender === 'בת' ? 'bg-rose-400 text-white' : 'bg-white dark:bg-gray-800'}`}>בת</button>
              </div>
              <input type="date" value={babyDob} onChange={e => { setBabyDob(e.target.value); localStorage.setItem('bobo_dob', e.target.value); }} className="w-full p-2 border rounded-xl font-bold text-center bg-white dark:bg-gray-800" />
            </div>
      ...
      {editingItem && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-3xl p-5 shadow-2xl space-y-4">
            ...
            <input type="datetime-local" value={editTimestamp} onChange={e => setEditTimestamp(e.target.value)} className="w-full p-2 border rounded-xl text-xs font-mono text-center bg-white dark:bg-gray-700" />
            <input type="text" value={editNotes} onChange={e => setEditNotes(e.target.value)} placeholder="הערות..." className="w-full p-2 border rounded-xl text-xs bg-white dark:bg-gray-700" />
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => setEditingItem(null)} className="py-2.5 bg-gray-100 dark:bg-gray-700 rounded-xl font-bold text-xs">ביטול</button>
      ...
      {showReportModal && (
        <div className="fixed inset-0 bg-gray-900/95 z-[100] flex flex-col p-4 overflow-y-auto">
          <div className="w-full max-w-lg mx-auto flex justify-between items-center mb-4">
            <h2 className="text-white font-bold">דוח התפתחות לרופא</h2>
            <button onClick={() => setShowReportModal(false)} className="text-white font-bold text-lg">✕</button>
          </div>
          <div className="bg-white rounded-2xl p-6 text-gray-900 space-y-4 max-w-lg mx-auto w-full">
            ...
            <button onClick={() => window.print()} className="w-full py-3 bg-teal-500 text-white font-bold rounded-xl text-xs shadow-md">🖨️ הדפס דוח</button>
          </div>
        </div>
      )}
    </div>
